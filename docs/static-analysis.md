...
| `controlled-delegatecall` | `GlauxDelegate.initialize` | Delegatecall to a committed target is what a proxy is. The function id is a hardcoded literal, not input; the target is bound by the birth proof and by the code-hash and compatibility-marker checks performed immediately before. |
...
| `abi.encodePacked()` Hash Collision | `GlauxDelegate` constructor, `GlauxDelegate.sol:59` | **False positive, same shape as the one above and for the same reason.** The three operands of `AUTH_MSG_HASH` are `hex"05d78094"` (4 bytes), `address(this)` (20) and `hex"80"` (1) — all fixed width, none caller-supplied, the result always exactly 25 bytes, so distinct inputs cannot encode alike. It is also not an encoding this project chooses: it is the EIP-7702 authorization preimage `0x05 ‖ rlp([0, router, 0])`, whose bytes are fixed by the protocol. Hashing anything else would produce an address no chain agrees with. New in the rootless-birth change, which put this constant in the router. |
